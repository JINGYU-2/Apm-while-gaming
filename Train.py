...
y_out = np.array([0]*100 + [1]*100 + [2]*100 + [3]*100)
...
X_in = np.column_stack((stdarr, numoutlier, DxDtchange))

# Convert to tensors
X_in = torch.from_numpy(X_in).type(torch.float32)
y_out = torch.from_numpy(y_out).type(torch.long)


# Split into train and test sets
X_in_train, X_in_test, y_out_train, y_out_test = train_test_split(
   X_in, y_out, test_size=0.2
)
...
# Training loop
epochs = 1000
X_blob_train, y_blob_train = X_in_train.to(device), y_out_train.to(device)
X_blob_test, y_blob_test = X_in_test.to(device), y_out_test.to(device)
